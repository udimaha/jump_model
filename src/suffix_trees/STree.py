...
class STree:
    ...
    def _create_node(self, x: str, u: "_SNode", d: int):
        i = u.idx
        p = u.parent
    ...
    def _create_leaf(self, x: str, i: int, u: "_SNode", d: int):
        w = _SNode()
        w.idx = i
        w.depth = len(x) - i
    ...
    def _compute_slink(self, x: str, u: "_SNode"):
        d = u.depth
...
class _SNode:
    ...
    def __init__(self, idx=-1, parent: Optional["_SNode"] = None, depth=-1):
        # Links
    ...
    def branch(self) -> str:
        ...
        res = self.parent.branch + me[0]
        if self.is_leaf():
            res = res[:-1]
        return res

    @property
    def suffix_link(self) -> "_SNode":
    ...
    def _get_transition_link(self, suffix: str) -> Union[bool, "_SNode"]:
        return False if suffix not in self.transition_links else self.transition_links[suffix]

    def _add_transition_link(self, snode: "_SNode", suffix: str):
